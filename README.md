...
마이홈포털의 주거복지 FAQ 496건(임대주택 · 공공분양 · 주택금융 · 공공기숙사)을 근거로
질문에 답하는 **Advanced RAG** 웹 서비스입니다.

사람들이 실제로 궁금해하는 내용을 DB에 담기 위해, [마이홈포털](https://www.myhome.go.kr)에 게시된
FAQ를 **웹 크롤링을 통해 취득**했습니다 (강의에서 배운 크롬 확장프로그램 활용).
크롤링 시 결과를 **계층 구조를 가진 Markdown(MD) 파일**로 저장했고,
이 계층형 MD 파일을 활용해 FAQ 문항 단위의 **계층형 청크**로 가공한 뒤 Vector DB에 적재했습니다.
...
**예시 질문**: "다자녀가구 특별공급 당첨자 선정방법은?" · "디딤돌대출 우대금리 요건은?" · "행복주택 입주 자격이 궁금해요"

## 과제 구현 항목

| 항목 | 구현 내용 |
|---|---|
| 문서 | 마이홈포털 주거복지 FAQ 496건 (크롬 확장프로그램으로 웹 크롤링, 2026-07-05 수집) |
| 청킹 | 크롤링한 계층형 MD 파일 → FAQ 문항 단위 계층형 청킹 → 506개 chunk |
...
├─ data/chunks.json            # FAQ chunk 506개
...
- ✅ FAQ 506개 chunk 임베딩 및 Supabase 적재 완료
- ✅ 벡터 검색 정확도 확인 (예: "다자녀가구 특별공급 당첨자 선정방법" → 해당 FAQ 1위, similarity 0.86)
- ✅ Rerank 동작 확인 (1위 rerank_score 0.95)
...
- **출처**: [마이홈포털](https://www.myhome.go.kr) 자주 묻는 질문(FAQ) · 임대주택(404) · 주택금융(44) · 공공분양(29) · 공공기숙사(19)
- **수집 방법**: 사용자들이 실제로 궁금해하는 질문·답변을 RAG DB에 넣기 위해, 강의에서 배운
  **크롬 확장프로그램을 활용한 웹 크롤링**으로 마이홈포털 FAQ 게시판 전체(496건)를 수집 (2026-07-05)
- **가공 절차**:
  1. 크롤링 결과를 **계층 형태의 Markdown(MD) 파일**로 저장 (질문/답변/분류/출처를 제목 계층으로 구조화)
  2. 계층형 MD 파일을 활용해 **FAQ 문항 단위의 계층형 청크**로 분할 (긴 답변은 overlap 분할 적용 → 506개 chunk)
  3. 각 chunk를 임베딩하여 Supabase pgvector에 적재
